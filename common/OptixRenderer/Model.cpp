...
int addVertex(TriangleMesh *mesh,
			  ...
			  map<tinyobj::index_t, int, cmp> &knownVertices)
{
				...
				const string &modelPath)
{
	...
	{
		cout << GDT_TERMINAL_RED
			 << "Could not load texture from " << fileName << "!"
			 << GDT_TERMINAL_DEFAULT << endl;
	}
...
Model *loadOBJ(const string &objFile)
{
	Model *model = new Model;

	const string modelDir = objFile.substr(0, objFile.rfind('/') + 1);
	...
	if (!readOK)
	{
		throw runtime_error("Could not read OBJ model from " + objFile + " : " + err);
	}

	if (materials.empty())
		throw runtime_error("could not parse materials ...");

	cout << "Done loading obj file - found " << shapes.size() << " shapes with " << materials.size() << " materials" << endl;
	map<string, int> knownTextures;
	for (int shapeID = 0; shapeID < (int)shapes.size(); shapeID++)
	{
		...
		for (int materialID : materialIDs)
		{
			...
			for (int faceID = 0; faceID < shape.mesh.material_ids.size(); faceID++)
			{
				...
				mesh->trans = (const vec3f &)materials[materialID].transmittance;
				mesh->ior = (float)materials[materialID].ior;
	...
	cout << "created a total of " << model->meshes.size() << " meshes" << endl;
	return model;
}

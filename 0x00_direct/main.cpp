...
int main(int ac, char **av)
{
	...
	std::string mod2 = "../../models/cbox_glossy/CornellBox-Glossy-Floor.obj";
	...
	try
	{
		Model *model = loadOBJ(mod0);
		Camera camera = {/*pos*/ vec3f(0.0f, 0.0f, 5.0f),
							/* forward*/ model->bounds.center() - vec3f(0.0f, 0.0f, 5.0f),
							/* up */ vec3f(0.f, 1.f, 0.f)};

		SampleWindow *window = new SampleWindow("MineRay", model, camera);
		window->setMoveSpeed(2.5f);
		window->setRotateSpeed(0.15f);
		window->run();
	}
